...
class ListUsers extends Component<MyProps, MyState> {
  ...
  async componentDidMount() {
    ...
    var result: Array<ItemUser> = await axios.get<ServerResponse>("http://192.168.0.4:8000/api/users").then((item) => {
  ...
  listItem(item: ItemUser) {
      ...
      if (item.uriavatar == null) {
        ...
        var uriImg: string = "http://192.168.0.4:8000" + item.uriavatar;
        return <List.Item
                  title={item.username}
                  description={item.email}
                  onPress={() => {
